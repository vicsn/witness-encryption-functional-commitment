...
pub fn setup_random(witness_length: u64) -> ((Vec<G1Projective>, Vec<G2Projective>), Vec<Scalar>, Vec<Scalar>, Scalar) {
...
pub fn commit(ck: (Vec<G1Projective>, Vec<G2Projective>), α: Vec<Scalar>) -> (G1Projective, (Vec<Scalar>, Scalar)) {
    let n = α.len();
    let mut rng = rand::thread_rng();
...
pub fn open(ck: (Vec<G1Projective>, Vec<G2Projective>), d: (Vec<Scalar>, Scalar), β: Vec<Scalar>) -> G1Projective {
    let n = β.len();
...
pub fn verification_pairings(ck: (Vec<G1Projective>, Vec<G2Projective>), cm: G1Projective, op: G1Projective, β: Vec<Scalar>, y: Scalar, n: u64, α: Vec<Scalar>) -> (Gt, Gt, Gt) {
    ...
    let righthandside2 = Bls12::pairing(&ck.0[0].to_affine(), &(ck.1[(n - 1) as usize].to_affine()*y).to_affine());
...
mod test {
    ...
    fn do_test_fc(should_succeed: bool) -> bool {
        ...
        println!("test_identity*Scalar::one().double(): {:?}", test_identity*(Scalar::one().double()));

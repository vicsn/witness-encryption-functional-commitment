...
pub fn gen_Γ_linear_fc(cm: G1Projective, ck: (Vec<G1Projective>, Vec<G2Projective>)) -> DMatrix<BlsElement> {
    ...
    let Γ = DMatrix::from_row_slice(1, 2, vec![
        cm_b, u2,
    ].as_slice());
...
pub fn gen_Γ_conjunction(Γ: Vec<DMatrix<BlsElement>>) -> DMatrix<BlsElement> {
    ...
    DMatrix::from_row_slice(rows, columns, &[
        Γ[0][0], Γ[0][4], Γ[0][8],  gt0, g10, g20,
        Γ[0][1], Γ[0][5], Γ[0][9],  gt0, g10, g20,
        Γ[0][2], Γ[0][6], Γ[0][10], gt0, g10, g20,
        Γ[0][3], Γ[0][7], Γ[0][11], gt0, g10, g20,
        gt0, g10, g20, Γ[1][0], Γ[1][4], Γ[1][8],
        gt0, g10, g20, Γ[1][1], Γ[1][5], Γ[1][9],
        gt0, g10, g20, Γ[1][2], Γ[1][6], Γ[1][10],
        gt0, g10, g20, Γ[1][3], Γ[1][7], Γ[1][11]
    ])    
}

pub fn gen_Γ_disjunction(Γ: Vec<DMatrix<BlsElement>>, θ: Vec<DMatrix<BlsElement>>) -> DMatrix<BlsElement> {
    if Γ.len() != 2 {
    ...
    DMatrix::from_row_slice(rows, columns, &[
        gt0,     g10,     g20,      gt0,     gt0,     g10,     g20,      gt0,
        Γ[0][0], Γ[0][4], Γ[0][8],  θ[0][0], gt0,     g10,     g20,      gt0,
        Γ[0][1], Γ[0][5], Γ[0][9],  θ[0][1], gt0,     g10,     g20,      gt0,
        Γ[0][2], Γ[0][6], Γ[0][10], θ[0][2], gt0,     g10,     g20,      gt0,
        Γ[0][3], Γ[0][7], Γ[0][11], θ[0][3], gt0,     g10,     g20,      gt0,
        gt0,     g10,     g20,      gt0,     Γ[1][0], Γ[1][4], Γ[1][8],  θ[1][0],
        gt0,     g10,     g20,      gt0,     Γ[1][1], Γ[1][5], Γ[1][9],  θ[1][1],
        gt0,     g10,     g20,      gt0,     Γ[1][2], Γ[1][6], Γ[1][10], θ[1][2],
        gt0,     g10,     g20,      gt0,     Γ[1][3], Γ[1][7], Γ[1][11], θ[1][3]
    ])    
}
...
pub fn define_language_random()->((G1Affine, G2Affine, G1Affine, G2Affine), DMatrix<BlsElement>) {
    ...
    let Γ = DMatrix::from_row_slice(4, 3, vec![
        g1g2, g1_1, g2_1,
        gt_1, g1a, g2_1,
        gt_1, g1_1, g2a,
        h1h2, h1a, h2a
...
pub fn gen_hash_keys(Γ: DMatrix<BlsElement>) -> (DMatrix<BlsElement>, DMatrix<BlsElement>) {
    let mut rng = rand::thread_rng();
    let a_n = DMatrix::from_fn(Γ.nrows(), 1, |_r, _c| { BlsElement::Scalar(Scalar::random(&mut rng)) });
...
pub fn generate_word(base_gen:(G1Affine, G2Affine, G1Affine, G2Affine), msg1: String, msg2: String) -> ((Scalar, Scalar, G1Affine, G2Affine), (G1Projective, G1Projective, G2Projective, G2Projective)) {
    ...
    let word = (base_gen.0*r1, (base_gen.2*r1) + msg1_g1, base_gen.1*r2, (base_gen.3*r2) + msg2_g2);

    (witness, word)
}

pub fn gen_θ_linear_fc(ck: (Vec<G1Projective>, Vec<G2Projective>), y: Scalar) -> DMatrix<BlsElement> {
    let θ = Bls12::pairing(&ck.0[0].to_affine(), &(ck.1[ck.1.len() - 1]*y).to_affine());
    DMatrix::from_vec(1, 1, vec![
        BlsElement::Gt(θ),
    ])
}

pub fn gen_θ(word: (G1Projective, G1Projective, G2Projective, G2Projective), msg1: String, msg2: String) -> DMatrix<BlsElement> {
    let msg1_g1 = msg_to_g1(msg1);
    let msg2_g2 = msg_to_g2(msg2);

    DMatrix::from_vec(4, 1, vec![
        ...
        BlsElement::Gt(Bls12::pairing(&word.1.to_affine(), &word.3.to_affine()) - Bls12::pairing(&msg1_g1, &msg2_g2)),
    ])
}
...
pub fn gen_λ_linear_fc(element1: G2Projective, element2: G1Projective) -> DMatrix<BlsElement> {
    DMatrix::from_row_slice(2, 1, vec![
        BlsElement::G2Affine(element1.to_affine()),
        BlsElement::G1Affine(element2.to_affine()),
    ].as_slice())
}

pub fn gen_prover_hash(hp: DMatrix<BlsElement>, witnesses: Vec<(Scalar, Scalar, G1Affine, G2Affine)>, words: Vec<(G1Projective, G1Projective, G2Projective, G2Projective)>, witness_to_prove: Option<u32>) -> Gt {
    let mut λs: Vec<DMatrix<BlsElement>> = vec![];
    for i in 0..witnesses.len() {
        λs.push(DMatrix::from_row_slice(3, 1, vec![
    ...
    match witness_to_prove {
        None => {
            ...
            λ_final = nalgebra::DMatrix::from_row_slice(λs.len()*λs[0].len(), 1, &λ_final_vec.as_slice());        
        }
        Some(i) => {
            let mut λ_final_vec: Vec<BlsElement> = vec![];
            if i == 0 {
            ...
            λ_final = nalgebra::DMatrix::from_row_slice(2 + 2*λs[i as usize].len(), 1, &λ_final_vec.as_slice());            
        }
    }
...
mod test {
    ...
    fn sphf_pairing_success() {
        assert_eq!(do_sphf_pairing_test(true, "Hello".to_string(), "World".to_string()), true);
    }

    #[test]
    fn sphf_pairing_failure() {
        assert_eq!(do_sphf_pairing_test(false, "Hello".to_string(), "World".to_string()), false);
    }
    ...
    fn do_sphf_conjunction_test(should_succeed: bool, msg1: String, msg2: String) -> bool {
        ...
        let θ_conjunction = nalgebra::DMatrix::from_row_slice(θ1.len() + θ2.len(), 1, &θ_conjunction_vec.as_slice());
    ...
    fn sphf_conjunction_success() {
        assert_eq!(do_sphf_conjunction_test(true, "Hello".to_string(), "World".to_string()), true);
    }

    #[test]
    fn sphf_conjunction_failure() {
        assert_eq!(do_sphf_conjunction_test(false, "Hello".to_string(), "World".to_string()), false);
    }

    fn do_sphf_disjunction_test(witness_to_prove: Option<u32>, word_to_mangle: Option<u32>, msg1: String, msg2: String) -> bool {
        let (base1_gen, Γ1) = define_language_random();
        let (base2_gen, Γ2) = define_language_random();
        ...
        match word_to_mangle {
            None => {},
            Some(word_to_mangle) => {
                if word_to_mangle == 0 { // Derp the word if we need to fail
                    word1.1 = word1.1*w1.0;
    ...
    fn sphf_disjunction_success_1() {
        assert_eq!(do_sphf_disjunction_test(Some(0), None, "Hello".to_string(), "World".to_string()), true);
    }

    #[test]
    fn sphf_disjunction_success_2() {
        assert_eq!(do_sphf_disjunction_test(Some(0), Some(1), "Hello".to_string(), "World".to_string()), true);
    }

    #[test]
    fn sphf_disjunction_success_3() {
        assert_eq!(do_sphf_disjunction_test(Some(1), None, "Hello".to_string(), "World".to_string()), true);
    }

    #[test]
    fn sphf_disjunction_success_4() {
        assert_eq!(do_sphf_disjunction_test(Some(1), Some(0), "Hello".to_string(), "World".to_string()), true);
    }

    #[test]
    fn sphf_disjunction_failure_1() {
        assert_eq!(do_sphf_disjunction_test(Some(0), Some(0), "Hello".to_string(), "World".to_string()), false);
    }

    #[test]
    fn sphf_disjunction_failure_2() {
        assert_eq!(do_sphf_disjunction_test(Some(1), Some(1), "Hello".to_string(), "World".to_string()), false);
    }
}